 ...
 * Se monta sobre la vista del inventario, que es la primera que la usa.
...
test.describe('Barra superior de navegacion', () => {
  test('Cualquier pantalla del grupo presenta la barra con busqueda y las seis secciones', async ({
    page,
  }) => {
    await abrir(page);
  ...
  test('La seccion en curso queda marcada como activa', async ({ page }) => {
    await abrir(page);
  ...
  test('Un visitante que abre Mi Cuenta recibe unicamente la opcion de registro', async ({
    page,
  }) => {
    await abrir(page);
  ...
  test('La barra no introduce desplazamiento horizontal en la resolucion mas estrecha', async ({
    page,
  }) => {
