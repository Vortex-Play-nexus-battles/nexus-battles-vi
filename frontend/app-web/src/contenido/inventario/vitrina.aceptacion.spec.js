/**
 * SCRUM-321 - Pruebas de aceptacion de HU-INV-001, en navegador real.
 *
...
const PROPIETARIO = 'jugador-de-prueba';
...
async function conInventarioDe(page, totalElementos) {
  await page.route('**/api/v1/inventarios/**', async (ruta) => {
    ...
    await ruta.fulfill({
      ...
      body: JSON.stringify({
        elementos, numero, tamanio, totalElementos, totalPaginas,
...
test.describe('Vitrina del inventario', () => {
  ...
  test('La vitrina muestra dieciseis productos en la resolucion de referencia',
    async ({ page }) => {
  ...
  for (const { ancho, alto } of resoluciones) {
    test(`El contenido se reorganiza en ${ancho} x ${alto} sin perder legibilidad`,
      async ({ page }) => {
        ...
        const medidas = await page.evaluate((minimo) => {
          ...
          for (const t of document.querySelectorAll('.vitrina__producto')) {
            for (const texto of t.querySelectorAll('p')) {
              ...
              if (px < minimo) problemas.push(`fuente ${px}px`);
  ...
  test('Un inventario vacio muestra un estado explicativo',
    async ({ page }) => {
      await page.setViewportSize({ width: 1360, height: 768 });
      ...
      await expect(page.locator('.estado-vacio')).toContainText(
        /todav[ií]a no tienes productos/i);
