...
//   node emitir-token.mjs <clave-privada.pem> --azp ms-subastas        (token de servicio)
//
// El token lleva realm_access.roles (lo que lee SeguridadConfig de productos)
// y vence en --horas. Con --azp emite un token de SERVICIO (como el de
// client_credentials de Keycloak): claim azp con el cliente, sin roles de
// usuario; es lo que inventario exige a ms-subastas en HU-INV-010. Se pega en la variable "token" de Postman o se pasa a
...
if (!ruta || ruta.startsWith('--')) {
  console.error('uso: node emitir-token.mjs <clave-privada.pem> [--rol ROL] [--usuario NOMBRE] [--horas N] [--azp CLIENTE]');
  process.exit(2);
}
...
const cuerpo = b64(azp
  ? { iss: 'jwks-dev', sub: azp, azp, realm_access: { roles: [] }, iat: ahora, exp: ahora + horas * 3600 }
  : { iss: 'jwks-dev', sub: usuario, preferred_username: usuario, realm_access: { roles: [rol] }, iat: ahora, exp: ahora + horas * 3600 });
