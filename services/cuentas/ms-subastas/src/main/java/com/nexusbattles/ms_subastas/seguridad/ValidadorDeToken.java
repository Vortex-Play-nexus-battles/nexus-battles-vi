...
public class ValidadorDeToken {
    ...
    public IdentidadDelSolicitante validar(String token) {
        ...
        return new IdentidadDelSolicitante(apodo, rol, version, leerJugadorId(claims));
    }

    /**
     * Lee el claim {@code uid} sin exigirlo. Su ausencia NO invalida el token:
     * ms-identidad lo omite a proposito cuando el usuario no tiene todavia un
     * identificador publico, y negarle el acceso por eso dejaria fuera al
     * listado publico, que no lo necesita. Quien si lo necesita lo exige con
     * {@link IdentidadDelSolicitante#exigirJugadorId()}.
     *
     * <p>Un {@code uid} presente pero que no es un UUID si invalida el token:
