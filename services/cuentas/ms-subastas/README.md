...
- **Controladores REST**: bloqueados por una decision de autenticacion. Copiar el `X-User-Id` de ms-ecommerce seria una vulnerabilidad aqui (cualquiera podria pujar en nombre de otro jugador y moverle los creditos). El interceptor JWT de ms-identidad no se puede importar: depende de sus entidades internas y ArchUnit prohibe cruzar dominios. Decidido con Cristian (HU-SUB-011) — ver "Asunciones tomadas", punto 4 — que la firma y expiracion se validan con un componente nuevo en `shared/libs/`; la verificacion de revocacion de rol queda fuera de alcance por ahora, documentada como riesgo aceptado.
