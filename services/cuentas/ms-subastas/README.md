...
- Pruebas unitarias de las 5 reglas + prueba de concurrencia (`MotorPujasServiceConcurrenciaTest`) que reproduce el caso pedido por el backlog: varios jugadores pujando en la misma fraccion de segundo, solo uno gana.

## Pendiente (bloqueado por otros, o fuera de este paquete)

- Repositorios JPA reales de `Subasta`/`Puja` y el lock pesimista de base de datos (hoy el `synchronized` de `MotorPujasService` solo protege pruebas en memoria de un solo proceso).
- `CreditoClientHttp` real contra el endpoint de ms-finanzas (Juan Diego, Dia 1-2).
