...
3. **Revisión obligatoria** del responsable del área, que GitHub solicita solo
   por CODEOWNERS.
4. **Solo squash-merge**: un commit por historia en el historial.
...
Angular imponía una estructura única. JavaScript plano no impone nada, así que
estas convenciones son obligatorias y se revisan en el pull request:
...
- **No hay wrapper de Gradle** ni ningún servicio de Spring Boot. Llegan en el
  siguiente pull request.
- **No hay archivos de Docker Compose.** Las bases de datos y la cola se añaden
  con el primer servicio.
- **No hay configuración de depuración de Java**, por lo mismo.

Lo que sí funciona hoy: abrir el repositorio, tener el mismo formato que todos,
editar HTML, CSS y JavaScript con revisión de tipos y linter, y trabajar con Git
