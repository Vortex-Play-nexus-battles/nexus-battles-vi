...
| D-14 | **Qué sanción silencia (RF-USR-004).** `moderacion-sanciones-consulta.yaml` solo dice `sancionActiva: true/false`, sin tipo (silencio, suspensión, expulsión). | **Cualquier sanción activa silencia** el chat (sala y general) y la publicación de comentarios. Si sanciones no responde, no se publica (503 `sanciones-no-disponibles`), igual que con la lista negra. | `salas-partidas`: `chat.integracion.ClienteSanciones`; `comentarios`: `publicacion.ClienteSanciones`. Cuando el contrato traiga tipo, cambian solo esas dos clases. | #33, #34, #441 |
